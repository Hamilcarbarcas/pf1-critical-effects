...
/* The pool's authoring fields (rank / slots / damageTypes / pins) are placement instructions and
 * stop here; everything else is content and travels through to the catalog untouched.
 *
 * The optional channels are omitted rather than nulled when empty, because an absent field and an
 * empty one mean the same thing to every consumer and the shorter one is easier to read in a
 * 6000-line generated file. `onFail` is the exception in kind rather than in treatment: an explicit
 * `null` INSIDE it is meaningful (§6 — it clears a channel on the failed branch), so the object is
 * copied through as authored and only its absence is elided. */
...
for (const entry of pool.entries) {
  ...
  /* The mechanical channels, checked with the SAME functions the runtime validates the generated
   * catalog with. At load those are warnings and a bad row is dropped; here they are problems and
   * nothing is written, which is the right asymmetry — a mistake caught while authoring is a typo
  ...
  if (entry.save == null && entry.onFail != null) {
    notes.push(`"${entry.id}": has \`onFail\` but no \`save\`, so the failed branch is unreachable`);
  }
