...
content/mortal.md       ← the 13+ addendum. GENERATED by the import; 47 rows.
...
| `node tools/verify.mjs` | run the *runtime* validators over the generated files. |
| `node tools/merge-flavor.mjs --write` | `tools/flavor.json` → the pool's `text` fields. |
| `node tools/buff-manifest.mjs` | spreadsheet → `BUFFS.md`, the compendium-buff worklist. |
| `node tools/csv-to-pool.mjs --write` | **one-off.** The spreadsheet import that built the pool. |
...
```jsonc
{
  ...
  "buffs": ["Broken Arm"],             // buff NAMES, delivered by astora-mod
...
A **mortal** entry is also unranked, but for the opposite reason — it sits past row 12 rather than
waiting for a score. It carries `mortal: true` so the coverage report can tell "deliberately
outside the ladder" from "waiting to be triaged", which are the same `rank: null` and opposite
situations.
...
**Mortal.** The 13+ addendum lives in [mortal.md](mortal.md) as **two tables** keyed by different
axes — and that split is the thing to get right when you write one:
...
{ "id": "dropped-weapon", "name": "Dropped Weapon",
  ...
  "text": null, "note": null, "buffs": [], "conditions": [] }
```
...
### `inherits` — "as 12, but worse"

Most 13+ results don't describe their own mechanics. They describe a *modification* of row 12, and
row 12 is a different wound in every cell they cover. So they are written as a transform instead:

```jsonc
{ "id": "mortal-blow", "name": "Mortal Blow", "rank": null, "mortal": true,
  "slots": ["*/head", "*/torso"], "damageTypes": ["bludgeoning", "piercing", "slashing"],
  "inherits": "row12", "transform": { "bleed": 2, "saveDC": 2, "scaleDH": true } }
```

That one entry covers **eighteen** cells, taking its buff, conditions and failed branch from
whatever sits in row 12 of each — Caved Cranium here, Punctured Heart there. The whole vocabulary:

| Op | Does |
|---|---|
| `bleed: n` | multiplies every bleed's **dice count**, and its `deep` threshold with it |
| `saveDC: n` | multiplies the parent's save multiplier |
| `scaleDH: true` | scales the inherited *buff's* healing threshold too — bakes as `dhScale` |
| `damage: [...]` | damage parts **added** to the parent's |
| `conditions: [...]` | conditions **added** to the parent's |
| `buffs: [...]` | buffs that **replace** the parent's |

Resolution happens at fold-in, so `effects.json` still ships one explicit entry per cell and the
engine never chases a reference. Two things to know:

- **Row 12 can move.** Placement is global, so adding a grave effect can re-seat it and change what
  a 13+ result inherits. `effects.json` carries a `_mortalParents` map so that shows up in the diff.
- **A transform with nothing to act on is reported**, not silently ignored — doubling the bleed of a
  row 12 that has no bleed names a mechanic the result doesn't have.
...
`src/catalog/schema.mjs` is the full list, plus `COMPANION_CONDITIONS` for the ones a companion
module registers — `burning` comes from pf1-bleed-effects, not from the system).
...
**Bleed is the exception.** Its magnitude never touches the Active Effect — pf1-bleed-effects keeps
a list of instances in an actor flag and groups them by kind when they tick — so hit-point bleed and
constitution bleed run side by side off one marker. `4d6 deep bleed` plus `2d4 con bleed` is one
wound bleeding two ways, and eight row-12 entries are written that way.

### Where a bleed goes

Follow the debuff it travels with, not the wound:

| The penalty is | The bleed goes | Because |
|---|---|---|
| **timed** — "1d4 rounds" | in `conditions` | the buff expires out from under it |
| **until healed** | on the **buff**, as pf1-bleed-effects' bleed flag | clearing the wound clears the bleeding |
| ability bleed, either way | in `conditions` | it isn't gated by the limb |

A buff carrying a bleed takes a dedicated-healing threshold instead of a `deep` value, and the
threshold is **derived**: 5 hit points per die of hit-point bleed, 10 per die of ability bleed. See
[BUFFS.md](BUFFS.md), which lists every buff the catalog references with its bleed and threshold.
...
### `buffs` — the buff NAMES

Not uuids: names, which is what astora-mod's `applyBuffTo` takes. A name survives a pack recompile,
finds a copy already on the actor (which is what makes Refresh work), and lets a GM keep their
buffs in a pack of their own.

**An array, like `conditions`, and for the same reason** — the content delivers more than one.
Thirteen entries pair a wound with *Weapon Stuck*: the steel is still in the victim, and that is a
second buff with its own lifetime, not a footnote on the first. Order matters only for
presentation: the first name leads the Apply button's label, so write the wound before the thing
holding it open.

```jsonc
"buffs": ["Pierced Knee", "Weapon Stuck"]
```

Don't name the same buff twice — `applyBuffTo` finds the copy already on the actor and refreshes
it, so the second is a no-op that reads like a mechanic. The validator reports it.
